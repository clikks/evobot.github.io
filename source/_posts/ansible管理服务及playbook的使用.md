...
- 上面的playbook中，`gather_facts`用来收集客户机上的一些属性信息，这里将其关闭,在机器数量较多时，建议关闭，减轻ansible压力，`vars`用来定义变量，变量名为`user`，值为`"test"`以便在后面进行引用；然后在tasks中使用了`user`模块，并且使用`{{}}`引用之前定义的变量`user`。
