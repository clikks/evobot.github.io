...
- 定义数组，语法为`a=(1 2 3 4 5 6)`或者`a=(1 'string' 3 4)`，打印数组，则是使用`echo $\{a[@]\}`或者`echo $\{a[*]\};

- 查看数组其中某个元素的值，则使用元素的下标，即元素在数组中是第几个，从0开始计数，例如`echo $\{a[1]\}`、`echo $\{a[0]\}`;

- 如果要获得元素中所有元素的个数，则使用`echo $\{#a[*]\}`
...
- 数组分片就是从数组中截取某几个元素，形式为`echo $\{a[*]:0:3\}`，表示截取数组a中从下标0开始之后的3个元素：
...
- 对数组中的元素的值进行替换，则使用`echo $\{b[*]/8/88\}`，对单个元素进行替换的效果与单独赋值是相同的，但当数组中存在相同值的元素时，替换能够将相同值的元素一次全部替换：
